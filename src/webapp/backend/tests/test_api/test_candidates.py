...
def test_update_candidates_success(client, mock_fs_service):
  ...
  mock_fs_service.update_candidates.assert_called_once()
  args, _ = mock_fs_service.update_candidates.call_args
  assert len(args[0]) == 1
  assert isinstance(args[0][0], candidate_model.Candidate)
  assert args[0][0].video_analysis_uuid == "va-123"


def test_insert_submission_requests_success(client, mock_fs_service):
  ...
  mock_fs_service.insert_submission_requests.assert_called_once_with(
      [expected_request]
  )


def test_insert_submission_requests_multi_destinations(client, mock_fs_service):
